...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        output = feat.reshape(feat.shape[0], np.prod(feat[0,:].shape))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        #db = 1*dprev
        self.grads[self.b_name] = np.dot(np.ones((1,dprev.shape[0])), dprev)
        self.grads[self.b_name] = self.grads[self.b_name].reshape(self.output_dim,)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class leaky_relu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        output = (feat<0) * feat * self.negative_slope + (feat >= 0) * feat
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        grad = (feat<0) * self.negative_slope + (feat>=0) * 1
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if (is_training==True) and (self.keep_prob > 0):
                #perform dropout
                mask = self.rng.rand(*feat.shape)
                mask = (mask <= self.keep_prob) #0/1 indicator
                #scale values with (1 / keep_prob)
                kept = mask / self.keep_prob
        else:
            #no dropout performed
            kept = np.ones((feat.shape)).astype(int)  
    ...
    def backward(self, dprev):
        ...
        if (self.is_training==True) and (self.keep_prob > 0):
            #perform gradient with dropout
            dfeat = self.kept * dprev
        else:
            #gradient without dropout
            dfeat = np.ones((dprev.shape)) * dprev    
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        log_p = np.log(logit)
        label_one = np.zeros((feat.shape))
        #put 1 into label position
        i=0
        for label_now in label:
            label_one[i,label_now] = 1
            i = i+1
        label_sum = -np.sum(np.multiply(label_one, log_p))
        loss = label_sum / (label.shape[0])
    ...
    def backward(self):
        ...
        label_one = np.zeros((logit.shape))
        i=0
        for label_now in label:
            label_one[i,label_now] = 1
            i = i+1
        dlogit = (logit - label_one) / (label.shape[0])
       # print("dlogit", dlogit.shape)
