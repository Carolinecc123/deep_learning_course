...
class Model:
    ...
    def forward(self, inputs):
        ...
        exp_sum = np.sum(np.exp(q), axis = 0) #sum of exponential, size 1 * batch_size
        softmax_q = np.exp(q) / exp_sum  #softmax function result 10 * batch_sizes
...
def main():
    ...
    sample_data = test_data[range(30, 40)]
    sample_label = test_label[range(30, 40)]
